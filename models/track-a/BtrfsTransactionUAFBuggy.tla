------------------------ MODULE BtrfsTransactionUAFBuggy ------------------------
EXTENDS Integers, Sequences, TLC

(*
This model demonstrates the CVE-2025-21753 Use-After-Free bug in btrfs join_transaction.
The bug occurs because join_transaction drops the trans_lock before reading the `aborted`
field of the transaction object, without holding a reference count on the object.
A concurrent abort can free the transaction object.
*)
...
\* Writer attempts to join transaction (Buggy version)
WriterJoinStep1(w) ==
    /\ writer_state[w] = "Idle"
    /\ trans_lock = FALSE
    /\ running_trans_id /= 0
    ...
    \* BUG: Does NOT bump refcount before leaving this atomic step!
    /\ UNCHANGED <<heap_allocated, heap_refcount, heap_aborted, running_trans_id, aborter_state, trans_lock>>

WriterJoinStep2(w) ==
    /\ writer_state[w] = "Joining"
    \* Reads the aborted field of the transaction it has a pointer to.
    \* If heap_allocated is FALSE, this is a Use-After-Free!
...
CleanupTransaction ==
    /\ \E id \in {i \in 1..MaxTransactions : heap_allocated[i] = TRUE /\ heap_refcount[i] = 0} :
